...
const Login = () => {
  ...
  // LOGIN
  const InputLogin = (tipo:string, e:React.ChangeEvent<HTMLInputElement>)=>{
    if (tipo=='username'){
  ...
  const LoginUser = (e:React.FormEvent<HTMLFormElement>)=>{
    ...
    const emailExists = usersList.some(user => user.email == nameLog);
    const nameExists = usersList.some(user => user.username == nameLog);
        ...
        {haveAccount ? (
          <div className="login-container centro">
            <form className="login-form" onSubmit={LoginUser}>
              <h2>BlogRooms</h2>
              <input onChange={(e)=>InputLogin('username', e)} type="text" placeholder="Username or email"></input>
              <input onChange={(e)=>InputLogin('password', e)} type="password" placeholder="Password"></input>
